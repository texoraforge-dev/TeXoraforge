...
export function AdmitStudentModal({
  ...
  // Auto generate credentials and auto-inherit class subjects when opening modal
  useEffect(() => {
    if (isOpen) {
      if (existingStudent) {
        ...
        const stdSubjects = existingStudent.enrolledSubjects && existingStudent.enrolledSubjects.length > 0
          ? existingStudent.enrolledSubjects
          : actions.getClassSubjects(existingStudent.classId);
        setEnrolledSubjects(stdSubjects);
      } else {
        const yr = new Date().getFullYear();
        const randNum = Math.floor(100 + Math.random() * 900);
        const codeSuffix = Math.floor(1000 + Math.random() * 9000);
        const initialClassId = classes[0]?.id || '';
        const initialClassSubjects = initialClassId ? actions.getClassSubjects(initialClassId) : availableSubjects;
        ...
        setEnrolledSubjects(initialClassSubjects);
      }
    }
  }, [isOpen, existingStudent, classes, school]);

  // When class changes during new student admission, auto-update enrolled subjects to the newly selected class's curriculum
  const handleClassChange = (newClassId: string) => {
    setClassId(newClassId);
    if (!existingStudent) {
      const newClassSubjects = actions.getClassSubjects(newClassId);
      setEnrolledSubjects(newClassSubjects);
    }
  };
                ...
                >
                  {classes.map((cls) => (
                    <option key={cls.id} value={cls.id}>
                      {cls.name} {cls.arm ? `(${cls.arm})` : ''} - {cls.category} ({(cls.subjects || []).length} subjects)
                    </option>
                  ))}
                </select>
              </div>
          ...
          <div className="space-y-3 pt-2 border-t border-slate-200 dark:border-slate-800">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400">
                  Auto-populated with {selectedClass?.name || 'Class'} subjects ({classCurriculumSubjects.length} subjects configured).
                </p>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => setEnrolledSubjects([...classCurriculumSubjects])}
                ...
                >
                  Reset to {selectedClass?.name || 'Class'} Curriculum
                </button>
                <button
                  type="button"
                  onClick={() => setEnrolledSubjects([...availableSubjects])}
                  className="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 text-[11px] font-bold hover:bg-slate-200 transition-colors"
                >
                  All School Subjects
                </button>
                <button
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 p-3 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 max-h-52 overflow-y-auto">
              {availableSubjects.map((sub) => {
                ...
                const isClassCurriculum = classCurriculumSubjects.includes(sub);
                return (
                  <div
                    key={sub}
                    onClick={() => {
                    ...
                    className={`flex items-center justify-between p-2 rounded-lg cursor-pointer text-xs font-semibold transition-all select-none border ${
                      isSelected
                  ...
                  >
                    ...
                    {isClassCurriculum && (
                      <span className="ml-1 text-[9px] font-extrabold px-1 py-0.2 rounded bg-indigo-100 dark:bg-indigo-900/80 text-indigo-700 dark:text-indigo-300 shrink-0">
                        Class
                      </span>
                    )}
                  </div>
